...
3. **Truncated-body semantics, amended during implementation review:** a prefix hit for `contains` is a Match; a prefix miss is Invalid and resolves by polarity; `equals` is Invalid on any truncated body. `regex` was originally grouped with `contains` (prefix hit = Match) but is amended to Invalid unconditionally on a truncated body, like `equals` — a regex match is not monotone under truncation (an end anchor or trailing `\b` can match the observed prefix and then fail once the rest of the body is accounted for), so a prefix hit is not sound evidence for the full body and would let an Allow rule grant on unseen bytes.
