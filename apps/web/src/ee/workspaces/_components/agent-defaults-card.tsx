...
/**
 * Which connections a brand-new agent in this workspace is granted
 ...
 * access after the agent exists. Every connection in the workspace's pool is
 ...
 */
export const AgentDefaultsCard = ({
  workspaceId,
  canManage,
}: AgentDefaultsCardProps) => {
  const connections = useConnections();
  const defaults = useAgentDefaults(workspaceId);
  const setDefault = useSetAgentDefault(workspaceId);
  const removeDefault = useRemoveAgentDefault(workspaceId);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          A brand-new agent in this workspace starts with these connections
      ...
      <CardContent className="space-y-3">
        ...
        {!isPending && (connections.data ?? []).length === 0 && (
          <p className="text-muted-foreground text-sm">
            Connect an app first: there&apos;s nothing to default a new agent to
            yet.
          </p>
        )}
        {!isPending &&
          (connections.data ?? []).map((conn) => (
            ...
            >
              ...
              <Switch
                ...
                aria-label={`Default ${conn.label ?? conn.provider} for new agents`}
              />
            </div>
          ))}
      </CardContent>
