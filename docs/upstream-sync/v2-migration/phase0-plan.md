...
- **Dead `!CAPS.rbac` arms** in free files (`middleware/auth.ts`, `services/workspace-access-check.ts`, `routes/org-skills.ts`, `routes/runners.ts`, `routes/org-channels.ts`, `apps/oauth-org.ts`, `channels/agent-channel-service.ts`, `slack/shared-install-service.ts`, web `lib/nav-config.ts`): now unreachable in every edition and all fail closed. Remove in a tidy PR after Phase 0 lands; each removal is a free-file edit to record. **DONE (Phase 2 WP-B tidy commit)** for the eight `packages/api` files. Web `lib/nav-config.ts` is the one exception: its `CAPS.rbac` (`apps/web/src/lib/env.ts`, no `entitled` arg) is a _different_, still-live flag — the client bundle never gets entitlement baked in by design (see CLAUDE.md and the doc-comment in `packages/api/src/lib/entitlements.ts`), so onprem's web build genuinely resolves `CAPS.rbac` to `false` until `useInstance()` learns the runtime entitlement. `nav-config.onprem.test.ts`/`nav-config.cloud.test.ts` pin both values. Left unchanged; this line item was wrong about it being dead.
...
From the final assembled-branch review:

- **WP4 placeholders that Phase 3 replaces with real UI:** `changeTeamMemberRole` throws "not available" although the API's `changeMemberRole` is real; `ManageAccessDialog`, `WorkspaceAccessDialog`, `WorkspaceAccessCard`, `ResourceScopeFields` are placeholders honouring the props contract; create-org, groups, domains, org general and app-availability pages are ComingSoon cards. Posture-equivalent to upstream-unlicensed (those routes returned 403 there), so acceptable for Phase 0.
- **Routes that now 404 rather than 403:** `/v1/org/members`, `/v1/org/groups`, `/v1/org/role-mappings`, `/v1/org/domains`, `/v1/workspaces/:id/access`, `/v1/org/app-availability`, `/v1/apps/dropbox/folders` — `registerEeRoutes` mounts nothing until Phase 2. The policy editor's identity picker therefore lists no users or groups yet.
- **Tidy PR after Phase 0 lands:** dead `!CAPS.rbac` arms (listed above) — **done, see the note above**; dead "Enterprise" copy behind `isLocked()` in the policy editor and workspace card, stale CI comments — still open.
- **Repo settings:** `release.yml` needs the `ONECLI_OSS_RELEASE` secret created on `whybutter/onecli`.
- **Decisions still open for Marco:** install/support domain replacing `onecli.sh` (decision 0.C; `TODO(fork-domain)` markers); whether "Request an app" keeps filing issues upstream; removal of `docker/docker-compose.legacy.yml` (decision 0.B).
- **Phase 6:** `docs/upstream-sync/state.json` `forkBase` → `v2.6.0`, `forkBranch` → `main` after cutover.

### Phase 0 free-file conflict surface

Files changed outside the three `ee/` roots, the licensing directory and docs (the set a future upstream merge can conflict on). Non-test: `.github/workflows/{publish,release,ci}.yml` (+ `cla.yml`, `ci-cache-seed.yml` deleted), `CLA.md` (deleted), `CLAUDE.md`, `CONTRIBUTING.md`, `LICENSE-ENTERPRISE` (deleted), `NOTICE`, `README.md`, `package.json`, `turbo.json`, `packages/api/package.json`, `apps/web/package.json`, `docker/{api,gateway,web,runner,agent,channel-adapter}.Dockerfile`, `docker/docker-compose.yml`, `scripts/{dev.mjs,install.sh}`, `apps/gateway/{Cargo.toml,Cargo.lock}`, `apps/gateway/crates/onecli-gateway/Cargo.toml`, `apps/gateway/crates/common/src/edition.rs`, `apps/gateway/crates/server/src/lib.rs` (comment), `apps/gateway-e2e/{README.md,src/env.ts,src/gateway.ts,src/scenario.ts}`, `apps/hosted-e2e/{README.md,src/api-server.ts,src/env.ts,src/gateway.ts}`, `packages/api/src/{edition-defaults.ts,lib/entitlements.ts,providers/hooks/policy-validator.ts,providers/hooks/rule-action-gate.ts,services/invitation-email.ts,testing/hermetic-env.ts}`, `packages/api/src/lib/entitlements-guard.ts` (deleted), `apps/web/src/app/(dashboard)/org/[orgId]/(admin)/{groups,settings/app-availability,settings/domains,settings/sso}/page.tsx`, `apps/web/src/app/auth/login/sso/page.tsx`, `apps/web/src/app/claim/page.tsx`, `apps/web/src/lib/auth/require-org-admin.ts` (new), `apps/web/src/lib/components/{coming-soon-card.tsx,feature-coming-soon-dialog.tsx}` (new; `enterprise-locked-card.tsx`, `license-required-dialog.tsx` deleted), `apps/web/src/lib/dashboard/{dashboard-header.tsx,sidebar-version.tsx}`, `apps/web/src/lib/install-command.ts`, `apps/web/src/lib/plan-gate.tsx`, `apps/web/src/lib/nav-config.ts`, `apps/web/src/lib/policy-editor/{resource-scope.tsx,resource-scope-types.ts}`. Plus roughly 37 free test files (deleted, rewritten or renamed) that asserted the unlicensed arm.
